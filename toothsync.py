"""
ToothSync - Dentist Patient Scheduling App
...
STORAGE_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), "appointments.json")

TREATMENTS = ["Cleaning", "Checkup", "Extraction", "Root Canal", "Filling", "Orthodontic"]
TIMES = [
...
def save_file(path, records):
    with open(path, "w") as f:
        json.dump(records, f, indent=4)


def make_record(name="Test Patient", phone="(555) 000-0001",
                email="test@clinic.com", date=None,
                time="09:00 AM", treatment="Cleaning",
                notes="", status="pending", uid=None):
    if date is None:
        date = datetime.now().strftime("%Y-%m-%d")
    if uid is None:
        uid = f"appt-{int(datetime.now().timestamp())}"
    return {
        "id":          uid,
        "patientName": name,
        "phone":       phone,
...
def validate_booking(name, phone, email, date, time, treatment):
    errors = []
    if not name.strip():
    ...
    if treatment == "Select Treatment *":
        errors.append("Treatment must be selected.")
    if date.strip():
        try:
...
def filter_records(records, query="", date_filter="", treatment_filter=""):
    q = query.lower().strip()
    result = []
    for r in records:
        if q and q not in r["patientName"].lower() and q not in r["phone"]:
            continue
        if date_filter and r["date"] != date_filter:
            continue
        if treatment_filter and treatment_filter != "All" and r["treatment"] != treatment_filter:
            continue
        result.append(r)
...
def get_unique_patients(records):
    unique = {}
    for r in records:
        key = r["patientName"].lower().strip()
...
class ToothSyncApp(tk.Tk):
    ...
    def _build_layout(self):
        ...
        for label_text, tab in nav_items:
            btn = tk.Button(
    ...
    def refresh_schedule(self):
        ...
        today = datetime.now().strftime("%Y-%m-%d")
        shown = filter_records(self.records, query=query)
        shown.sort(key=lambda x: (x["date"], x["time"]))

        self._stat_labels["today"].config(text=str(sum(1 for r in self.records if r["date"] == today)))
        self._stat_labels["pending"].config(text=str(sum(1 for r in self.records if r["status"] == "pending")))
        self._stat_labels["done"].config(text=str(sum(1 for r in self.records if r["status"] == "completed")))
        self._stat_labels["total"].config(text=str(len(self.records)))
    ...
    def _build_booking_tab(self):
        ...
        ttk.Combobox(row, textvariable=self.f_time_var, values=TIMES,
                     state="readonly", width=22, font=("Helvetica", 10)).pack(side="left")
    ...
    def refresh_patients(self):
        ...
        unique = get_unique_patients(self.records)
